...
//        printAllTasks(taskManager);
        System.out.println("\n======================LOADING======================");
        FileBackedTasksManager fromFile = FileBackedTasksManager.loadFromFile(new File("backup.csv"));
        System.out.println(fromFile.getAllEpicObjects());
        System.out.println(fromFile.getAllSubtaskObjects());
        System.out.println(fromFile.getAllTaskObjects());
        System.out.println("История просмотров: " + fromFile.getHistory().stream().map(Task::getId).collect(Collectors.toList()));
    }
